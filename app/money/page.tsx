...
const CHAPTERS = [
  { index: 0, number: 1, title: "타로카드 78장 금전 해석 레퍼런스", sections: 25 },
  { index: 1, number: 2, title: "금전 타로의 시작 — 질문법과 리딩 준비", sections: 4 },
  { index: 2, number: 3, title: "카드 위치와 조합 해석", sections: 4 },
  { index: 3, number: 4, title: "금전 상담 실전 완전 정복 I", sections: 4 },
  { index: 4, number: 5, title: "금전 상담 실전 완전 정복 II", sections: 5 },
];
...
export default function MoneyPage() {
  ...
  async function generateAll() {
    ...
    for (let ci = 0; ci < CHAPTERS.length; ci++) {
      ...
      try {
        const res = await fetch("/api/generate-money", {
          method: "POST",
...
<head>
<meta charset="UTF-8">
<title>3초만에 외워지는 마법의 금전백서</title>
<style>${allCss}</style>
</head>
<body>
${allHtml}
    ...
    <>
      <div id="ctrl">
        <div className="inner">
          <div className="hd">
            <h1>마법의 금전백서 생성기</h1>
            <p>타로카드 78장 금전 해석 + 실전 상담 완전 정복</p>
          </div>
      ...
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0;}
        body{background:#C4A84A;font-family:-apple-system,sans-serif;}
        #ctrl{position:sticky;top:0;z-index:100;background:#fff;border-bottom:1px solid #E0D090;padding:14px 20px;box-shadow:0 2px 12px rgba(0,0,0,0.08);}
        .inner{max-width:720px;margin:0 auto;display:flex;flex-direction:column;gap:11px;}
        .hd h1{font-size:16px;color:#1A1208;font-weight:700;margin-bottom:2px;}
        .hd p{font-size:11px;color:#8A6820;}
        .toc h3{font-size:9.5px;color:#8B6914;font-weight:600;letter-spacing:1px;text-transform:uppercase;margin-bottom:6px;}
        .toc ul{list-style:none;display:flex;flex-direction:column;gap:3px;}
        .toc li{display:flex;align-items:center;gap:8px;font-size:11.5px;color:#8A6820;padding:3px 8px;border-radius:4px;}
        .toc li.active{background:#FEFDF0;color:#8B6914;font-weight:600;}
        .toc li.done{color:#4A3510;}
        .ico{font-size:12px;flex-shrink:0;width:18px;}
        .prog{display:flex;flex-direction:column;gap:4px;}
        .bar{height:5px;background:#F0E8C0;border-radius:10px;overflow:hidden;}
        .fill{height:100%;background:#8B6914;border-radius:10px;transition:width 0.4s ease;}
        .lbl{font-size:10.5px;color:#8A6820;}
        .btns{display:flex;gap:9px;}
        .btn-main{padding:9px 20px;background:#8B6914;color:#fff;border:none;border-radius:6px;font-size:12.5px;font-weight:600;cursor:pointer;}
        .btn-main:hover:not(:disabled){background:#6B5010;}
        .btn-main:disabled{background:#C4A870;cursor:not-allowed;}
        .btn-dl{padding:9px 20px;background:#fff;color:#8B6914;border:1.5px solid #8B6914;border-radius:6px;font-size:12.5px;font-weight:600;cursor:pointer;}
        .btn-dl:hover{background:#FEFDF0;}
        .guide{font-size:10.5px;color:#4A3510;background:#EDE0C8;padding:8px 12px;border-radius:4px;line-height:1.7;}
