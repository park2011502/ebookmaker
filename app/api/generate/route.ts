...
function buildSummaryHtml(chapterNum: number, chapterTitle: string, summary: string[], quote: string, isLastChapter: boolean, pageNum: number): string {
  return `<div class="pg"><div class="pg-hd"><span>연애 프리패스 | 타로로 꿰뚫는 상대의 속마음</span><span>CH ${chapterNum} · 핵심 요약</span></div><div class="pg-body"><div class="sec-badge"><span class="sn">✦</span><span class="sl">Summary</span></div><div class="sec-title">CHAPTER ${chapterNum} 핵심 요약</div><div class="sec-rule"></div><div class="quote-box"><p>"${quote}"</p></div><ul class="summary-list">${summary.map(s=>`<li>${s}</li>`).join("")}</ul><div class="div-rule"></div><div class="tip-box"><div class="tip-title">${isLastChapter?"마치며":"다음 챕터 미리보기"}</div><p>${isLastChapter?"이 책을 통해 타로는 단순한 점술이 아닌, 나 자신과 상대방을 깊이 이해하는 도구임을 깨달으셨기를 바랍니다. 이제 어떤 연애 상담도 자신 있게 리딩할 수 있습니다.":`CHAPTER ${chapterNum+1}에서는 더 깊은 실전 리딩 기술을 배웁니다. 지금까지 배운 내용을 바탕으로 실제 상황에 바로 적용할 수 있는 해석법을 함께 알아볼게요.`}</p></div></div><div class="pg-ft"><div class="pn">${pageNum}</div><div class="pt">Tarot Love Guide</div></div></div>`;
}
