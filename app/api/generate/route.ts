...
const CHAPTERS = [
  ...
  {
    number: 4, title: "연애 상담 실전 완전 정복 I — 썸·연애·권태기",
    sections: [
...
.card-ref-img img{width:52px;border-radius:3px;box-shadow:2px 4px 12px rgba(80,20,20,0.22);display:block;}
...
.card-ref-label{font-family:var(--serif);font-size:8.5px;font-weight:600;color:var(--rose);margin-bottom:2px;}
...
.minor-card-top img{width:36px;border-radius:2px;box-shadow:1px 2px 6px rgba(80,20,20,0.18);flex-shrink:0;}
.minor-card-name{font-family:var(--display);font-size:9.5px;font-weight:700;color:var(--dark);line-height:1.3;margin-bottom:2px;}
.minor-card-kw{font-family:var(--sans);font-size:7.5px;color:var(--gold);font-weight:600;}
.minor-card-body{}
...
.sec-rule{width:28px;height:2px;background:var(--rose);margin-bottom:10px;flex-shrink:0;}
.card-callout{display:flex;gap:12px;background:var(--rose-faint);border:1px solid var(--rose-lt);border-radius:5px;padding:12px 13px;margin:9px 0;flex-shrink:0;}
...
.cc-body h4{font-family:var(--serif);font-size:12.5px;font-weight:600;color:var(--rose);margin-bottom:6px;}
.cc-body p{font-family:var(--sans);font-size:12px;color:var(--mid);line-height:1.9;word-break:keep-all;}
.cc-tagline{display:block;margin-top:5px;font-family:var(--serif);font-size:9px;font-style:italic;color:var(--rose-mid);}
...
.sub-h{font-family:var(--serif);font-size:13px;font-weight:600;color:var(--rose);margin:11px 0 6px;line-height:1.4;}
...
.bad-box{border-left:3px solid var(--rose-mid);background:#FDF4F6;border-radius:0 4px 4px 0;padding:9px 12px;flex:1;}
.good-box{border-left:3px solid var(--gold);background:var(--gold-lt);border-radius:0 4px 4px 0;padding:9px 12px;flex:1;}
...
.data-table th{background:var(--rose-pale);color:var(--rose);font-family:var(--serif);font-weight:600;padding:6px 9px;border:1px solid var(--rose-lt);text-align:center;font-size:10.5px;}
...
.quote-box{background:var(--rose);border-radius:5px;padding:10px 14px;margin:8px 0;flex-shrink:0;text-align:center;}
.quote-box p{font-family:var(--display);font-size:11.5px;font-style:italic;color:#fff;line-height:1.7;}
.tip-box{background:var(--gold-lt);border:1px solid #D0B080;border-radius:4px;padding:10px 13px;margin:8px 0;flex-shrink:0;}
...
.tip-box p{font-family:var(--sans);font-size:11.5px;color:#5C3A10;line-height:1.8;word-break:keep-all;}
...
.case-q{font-family:var(--sans);font-size:10px;color:var(--rose);font-style:italic;line-height:1.65;margin-bottom:5px;word-break:keep-all;}
...
.quiz-answer-box{background:var(--cream-mid);border-left:3px solid var(--rose);padding:8px 11px;border-radius:0 4px 4px 0;}
.quiz-answer-label{font-family:var(--sans);font-size:8px;font-weight:600;color:var(--rose);margin-bottom:4px;letter-spacing:1px;text-transform:uppercase;}
...
- 각 항목을 충분히 자세하게 작성하여 내용이 풍부하게 보일 것.
{
  "card1": {
    "name": "${card1.name}",
    "num": "${card1.num}",
    "keywords": "연애 핵심 키워드 3개 (예: 새로운 시작 · 설렘 · 충동)",
    "upright": "정방향 연애 해석. 이 카드가 연애 리딩에서 정방향으로 나왔을 때 의미를 구어체로 3문장 이상 충분히 설명. 반드시 마침표로 끝낼 것.",
    "reversed": "역방향 연애 해석. 이 카드가 역방향으로 나왔을 때 의미를 구어체로 3문장 이상 충분히 설명. 반드시 마침표로 끝낼 것.",
    "consult": "연애 상담에서 이 카드가 나오면 어떤 상황을 의미하는지 구어체로 2문장. 반드시 마침표로 끝낼 것."
  },
  "card2": {
    "name": "${card2.name}",
    "num": "${card2.num}",
    "keywords": "연애 핵심 키워드 3개",
    "upright": "정방향 연애 해석 3문장 이상. 반드시 마침표로 끝낼 것.",
    "reversed": "역방향 연애 해석 3문장 이상. 반드시 마침표로 끝낼 것.",
    "consult": "연애 상담 적용 2문장. 반드시 마침표로 끝낼 것."
  }
}
JSON만 출력.`;
}
...
- 각 항목 충분히 자세하게 작성할 것.
{
  "cards": [
    ${cards.map(c=>`{
      "name": "${c.name}",
      "keywords": "연애 핵심 키워드 2~3개",
      "upright": "정방향 연애 해석 2문장. 반드시 마침표로 끝낼 것.",
      "reversed": "역방향 연애 해석 2문장. 반드시 마침표로 끝낼 것."
    }`).join(",\n    ")}
  ]
}
JSON만 출력.`;
...
function buildSectionPrompt(chapterNum: number, chapterTitle: string, section: { title: string; card: string; cardName: string; extraCards: { file: string; name: string }[] }, si: number): string {
  ...
  if (chapterNum === 3) {
    ...
    const sub1Body = is3card
      ...
      : "2문장. 어떤 카드든 통용되는 위치/조합 해석 원칙을 메이저/마이너/역방향 구분 없이 압축. 반드시 카드 이름을 명시하며 예시를 들 것. 절대 이 카드 라고만 쓰지 말 것. 반드시 존댓말로, 마침표로 끝낼 것.";
    const sub2Body = is3card
      ...
      : "2문장. 컵/완드/검/펜타클 4개 원소가 각 위치에 올 때 연애 의미를 하나의 흐름으로 압축 설명. 4개 원소 모두 빠짐없이 카드명 명시. 절대 이 카드 라고만 쓰지 말 것. 반드시 마침표로 끝낼 것. 4개 원소 빠지는 것 없이.";
    ...
    {"question": "${section.title} 관련 실제 연애 상담 질문 2. 앞의 질문과 다른 연애 고민 상황", "answer": "2문장. 카드를 하나씩 나열하지 말고 전체 배열을 하나의 흐름으로 통합 해석. 모든 카드와 결론까지 반드시 포함. 내용이 풍부하고 빠지는 카드 없이. 반드시 존댓말로, 마침표로 끝낼 것."}`
      : `{"question": "${section.title} 관련 실제 연애 상담에서 자주 나오는 질문 1. 구체적 상황 포함. 3장 이상 배열 사용. 케이스 2,3번과 완전히 다른 상황으로", "answer": "3문장. 카드를 하나씩 나열하지 말고 위치별 원칙과 카드 의미를 하나의 흐름으로 통합 해석. 나온 카드 전부와 결론까지 빠짐없이. 반드시 존댓말로, 마침표로 끝낼 것."},
    {"question": "${section.title} 관련 실제 연애 상담에서 자주 나오는 질문 2. 케이스 1번과 완전히 다른 구체적 상황. 3장 이상 배열 사용.", "answer": "3문장. 카드를 하나씩 나열하지 말고 위치별 원칙과 카드 의미를 하나의 흐름으로 통합 해석. 나온 카드 전부와 결론까지 빠짐없이. 반드시 존댓말로, 마침표로 끝낼 것."},
    {"question": "${section.title} 관련 실제 연애 상담에서 자주 나오는 질문 3. 케이스 1,2번과 완전히 다른 구체적 상황. 3장 이상 배열 사용.", "answer": "3문장. 카드를 하나씩 나열하지 말고 위치별 원칙과 카드 의미를 하나의 흐름으로 통합 해석. 나온 카드 전부와 결론까지 빠짐없이. 반드시 존댓말로, 마침표로 끝낼 것."}`;
...
- 케이스와 퀴즈는 반드시 연애·관계·감정에 관한 내용으로만 작성할 것. 커리어·금전·자아성찰 내용 절대 금지.
{
  ...
  "subheadings": [
    ...
    {"title": "💬 소제목3 — 실전 적용법", "body": "2문장. 실제 연애 리딩에서 이 원칙 적용법을 카드명 명시하며 압축 설명. 절대 이 카드 라고만 쓰지 말 것. 반드시 마침표로 끝낼 것."}
  ],
  "extraCards": [
...
- 독자가 어떤 카드가 나와도 이 상황을 해석할 수 있도록 여러 옵션을 제공할 것.
...
- 카드 한국어명 주의: 황후→여황제, 행맨→매달린 사람, 검 왕→검 킹, The Fool→바보, Wheel of Fortune→운명의 수레바퀴.
{
  ...
  "subheadings": [
    ...
    {"title": "💡 소제목2 — 카드별 해석 옵션", "body": "2문장. 같은 연애 질문이라도 나오는 카드에 따라 해석이 달라지는 옵션 3가지 이상을 카드명 명시하며 하나의 흐름으로 압축. 예) OO카드가 나오면 ~이에요 형식으로. 절대 이 카드 라고만 쓰지 말 것. 반드시 존댓말로, 마침표로 끝낼 것."},
    {"title": "💬 소제목3 — 배열 위치별 적용", "body": "2문장. 이 연애 상황에서 3카드 또는 5카드 배열의 각 위치별 해석을 카드명 명시하며 하나의 흐름으로 압축. 절대 이 카드 라고만 쓰지 말 것. 반드시 마침표로 끝낼 것."}
  ],
  "extraCards": [
  ...
  "cases": [
    {"question": "${section.title} 관련 실제 연애 상담에서 자주 나오는 질문 1. 구체적이고 현실적인 연애 고민 상황 포함. 다른 케이스와 완전히 다른 상황으로", "answer": "존댓말로 4~5문장. 질문에서 언급된 카드나 이 섹션 주제에서 다루는 내용(원소/위치/카드 등)이 빠지는 것 없이 모두 포함. 반드시 구체적인 카드명을 언급하며 카드별 해석 3가지 이상 옵션 포함. 반드시 마침표로 끝낼 것"},
    {"question": "${section.title} 관련 실제 연애 상담에서 자주 나오는 질문 2. 앞의 질문과 완전히 다른 연애 상황으로", "answer": "존댓말로 4~5문장. 질문에서 언급된 카드나 이 섹션 주제에서 다루는 내용(원소/위치/카드 등)이 빠지는 것 없이 모두 포함. 반드시 구체적인 카드명을 언급하며 카드별 해석 3가지 이상 옵션 포함. 반드시 마침표로 끝낼 것"},
    {"question": "${section.title} 관련 실제 연애 상담에서 자주 나오는 질문 3. 앞의 두 질문과 완전히 다른 연애 상황으로", "answer": "존댓말로 4~5문장. 질문에서 언급된 카드나 이 섹션 주제에서 다루는 내용(원소/위치/카드 등)이 빠지는 것 없이 모두 포함. 반드시 구체적인 카드명을 언급하며 카드별 해석 3가지 이상 옵션 포함. 반드시 마침표로 끝낼 것"}
  ],
  "quiz": {
