...
function buildSectionHtml(
  ...
  html += `<div class="pg">${hd}<div class="pg-body">${badge}<div class="card-callout"><div class="cc-img"><img src="${BASE_URL}/cards/${sec.cardFile}" alt="${sec.cardName}"><div class="cc-name">${sec.cardName.replace(/\.+$/, "")}</div></div><div class="cc-body"><h4>💡 ${sec.cardName.replace(/\.+$/, "")}이 말하는 것</h4><p>${trimSentences(sec.cardDesc,6)}</p><span class="cc-tagline">${sec.cardTagline}</span></div></div>${(() => { const subBodyMax = (sec.title.includes("3카드 스프레드") || sec.title.includes("5카드 스프레드")) ? 2 : 2; return sec.subheadings.slice(0,2).map(sh=>`<div class="sub-h">${sh.title.replace(/\.+$/, "")}</div><p class="body-p">${trimSentences(sh.body,subBodyMax)}</p>`).join(""); })()}</div><div class="pg-ft"><div class="pn">${pg++}</div><div class="pt">Tarot Career Guide</div></div></div>`;
  ...
  const quizBlock = secWithQuiz.quiz
    ...
    : "";
  html += `<div class="pg"><div class="pg-hd"><span>마법의 직업백서 | 타로로 꿰뚫는 나의 커리어 흐름</span><span>CH 0${chapterNum} · 실전 상담 케이스</span></div><div class="pg-body"><div class="sec-badge"><span class="sn">0${si+1}</span><span class="sl">실전 상담 케이스</span></div><div class="sec-title">${safeTitle}</div><div class="sec-rule"></div>${sec.cases.slice(0,2).map((c,i)=>`<div class="case-box"><div class="case-title">💬 케이스 ${i+1}</div><p class="case-q">Q. ${c.question}</p><p class="case-a">A. ${(sec.title.includes("5카드 스프레드") ? trimSentences(c.answer,2) : trimSentences(c.answer,3))}</p></div>`).join("")}<div class="tip-box"><div class="tip-title">✦ 이 섹션 핵심 요약</div><p>${sec.summary.slice(0,2).join(" ")}</p></div>${quizBlock}</div><div class="pg-ft"><div class="pn">${pg++}</div><div class="pt">Tarot Career Guide</div></div></div>`;
