...
function buildSectionHtml(
  ...
  html += `<div class="pg">${hd}<div class="pg-body">${badge}<div class="card-callout"><div class="cc-img"><img src="${BASE_URL}/cards/${sec.cardFile}" alt="${sec.cardName}"><div class="cc-name">${sec.cardName.replace(/\.+$/, "")}</div></div><div class="cc-body"><h4>💡 ${sec.cardName.replace(/\.+$/, "")}이 말하는 것</h4><p>${trimSentences(sec.cardDesc,6)}</p><span class="cc-tagline">${sec.cardTagline}</span></div></div>${(() => { const subBodyMax = (sec.title.includes("3카드 스프레드") || sec.title.includes("5카드 스프레드")) ? 3 : 2; return sec.subheadings.slice(0,2).map(sh=>`<div class="sub-h">${sh.title.replace(/\.+$/, "")}</div><p class="body-p">${trimSentences(sh.body,subBodyMax)}</p>`).join(""); })()}</div><div class="pg-ft"><div class="pn">${pg++}</div><div class="pt">Tarot Career Guide</div></div></div>`;
  ...
  const quizBlock = secWithQuiz.quiz
    ...
    : "";
  html += `<div class="pg"><div class="pg-hd"><span>마법의 직업백서 | 타로로 꿰뚫는 나의 커리어 흐름</span><span>CH 0${chapterNum} · 실전 상담 케이스</span></div><div class="pg-body"><div class="sec-badge"><span class="sn">0${si+1}</span><span class="sl">실전 상담 케이스</span></div><div class="sec-title">${safeTitle}</div><div class="sec-rule"></div>${(() => { const caseMax = 2; return sec.cases.slice(0,caseMax).map((c,i)=>`<div class="case-box"><div class="case-title">💬 케이스 ${i+1}</div><p class="case-q">Q. ${c.question}</p><p class="case-a">A. ${(sec.title.includes("5카드 스프레드") ? trimSentences(c.answer,2) : trimSentences(c.answer,3))}</p></div>`).join(""); })()}<div class="tip-box"><div class="tip-title">✦ 이 섹션 핵심 요약</div><p>${sec.summary.slice(0,2).join(" ")}</p></div>${quizBlock}</div><div class="pg-ft"><div class="pn">${pg++}</div><div class="pt">Tarot Career Guide</div></div></div>`;
...
- 케이스와 퀴즈는 반드시 직업·커리어에 관한 내용으로만 작성할 것. 연애·금전 내용 절대 금지.

{
  ...
  "cases": [
    {"question": "${section.title} 관련 실제 커리어 상담 질문 1. 구체적 커리어 고민 상황 포함. 카드 3장 이상 배열 사용. 다른 케이스와 완전히 다른 상황으로", "answer": "2문장. 카드를 하나씩 나열하지 말고 전체 배열을 하나의 흐름으로 통합 해석. 모든 카드와 결론까지 반드시 포함. 내용이 풍부하고 빠지는 카드 없이. 반드시 존댓말로, 마침표로 끝낼 것."},
    {"question": "${section.title} 관련 실제 커리어 상담 질문 2. 앞의 질문과 완전히 다른 커리어 상황으로", "answer": "2문장. 카드를 하나씩 나열하지 말고 전체 배열을 하나의 흐름으로 통합 해석. 모든 카드와 결론까지 반드시 포함. 내용이 풍부하고 빠지는 카드 없이. 반드시 존댓말로, 마침표로 끝낼 것."},
    {"question": "${section.title} 관련 실제 커리어 상담 질문 3. 앞의 두 질문과 완전히 다른 커리어 상황으로", "answer": "2문장. 카드를 하나씩 나열하지 말고 전체 배열을 하나의 흐름으로 통합 해석. 모든 카드와 결론까지 반드시 포함. 내용이 풍부하고 빠지는 카드 없이. 반드시 존댓말로, 마침표로 끝낼 것."}
  ],
  "quiz": {
...

{
  ...
  "cardFile": "${section.card}",
  "cardName": "${section.cardName}",
  "cardDesc": "이 상황(${section.title})의 커리어 타로 리딩 포인트 5~6문장. 긍정/주의 카드 구체적으로 소개. 반드시 한국어 카드명 사용. 영어 카드명 절대 금지. 반드시 존댓말로, 마침표로 끝낼 것.",
  "cardTagline": "이 상황 커리어 리딩의 핵심 포인트 한 줄.",
  "subheadings": [
    {"title": "🔮 소제목1 — 이 상황에서 자주 나오는 카드들", "body": "5~6문장. 이 커리어 상황에서 긍정 신호 카드(예: 전차, 태양, 세계)와 주의 신호 카드(예: 탑, 달, 검 5, 매달린 사람)를 반드시 한국어 카드명으로 구체적으로 나열. 영어 카드명 절대 금지. 반드시 존댓말로, 마침표로 끝낼 것."},
    {"title": "💡 소제목2 — 카드별 해석 옵션", "body": "5~6문장. 같은 커리어 질문에 다른 카드가 나올 때 해석이 어떻게 달라지는지 반드시 카드명을 명시하며 3가지 이상 구체적으로 설명. 예) OO카드가 나오면 ~이에요 형식으로. 절대 이 카드 라고만 쓰지 말 것. 반드시 존댓말로, 마침표로 끝낼 것."},
    {"title": "💬 소제목3 — 배열 위치별 적용", "body": "5~6문장. 이 커리어 상황에서 3카드 또는 5카드 배열을 쓸 때 각 위치별로 어떻게 읽는지. 반드시 카드 이름을 명시하며 그 카드가 나왔을 때 어떻게 해석하는지 예시를 들 것. 절대 이 카드 라고만 쓰지 말 것. 반드시 마침표로 끝낼 것."}
  ],
  "extraCards": [
  ...
  "cases": [
    {"question": "${section.title} 관련 실제 커리어 상담에서 자주 나오는 질문 1. 구체적이고 현실적인 커리어 고민 상황 포함. 다른 케이스와 완전히 다른 상황으로", "answer": "존댓말로 4~5문장. 반드시 구체적인 카드명을 언급하며 카드별 해석 3가지 이상 옵션 포함. 반드시 마침표로 끝낼 것"},
    {"question": "${section.title} 관련 실제 커리어 상담에서 자주 나오는 질문 2. 앞의 질문과 완전히 다른 커리어 상황으로", "answer": "존댓말로 4~5문장. 반드시 구체적인 카드명을 언급하며 카드별 해석 3가지 이상 옵션 포함. 반드시 마침표로 끝낼 것"},
    {"question": "${section.title} 관련 실제 커리어 상담에서 자주 나오는 질문 3. 앞의 두 질문과 완전히 다른 커리어 상황으로", "answer": "존댓말로 4~5문장. 반드시 구체적인 카드명을 언급하며 카드별 해석 3가지 이상 옵션 포함. 반드시 마침표로 끝낼 것"},
    {"question": "${section.title} 관련 실제 커리어 상담에서 자주 나오는 질문 3. 앞의 두 질문과 완전히 다른 커리어 상황으로", "answer": "존댓말로 4~5문장. 반드시 구체적인 카드명을 언급하며 카드별 해석 3가지 이상 옵션 포함. 반드시 마침표로 끝낼 것"}
  ],
  "quiz": {
...
{"title":"${section.title}","cardFile":"${section.card}","cardName":"${section.cardName}","cardDesc":"7~8문장. 이 카드의 커리어 핵심 의미. 정방향과 역방향, 실제 직업 상황 예시, 독자가 바로 공감할 수 있는 구체적 내용 포함. 반드시 존댓말로, 마침표로 끝낼 것.","cardTagline":"한 줄.","subheadings":[{"title":"🔮 소제목1","body":"5~6문장. 이 섹션 주제 관련 핵심 커리어 내용을 구체적으로. 반드시 카드 이름을 명시하며 예시를 들 것. 절대 이 카드 라고만 쓰지 말 것. 반드시 존댓말로, 마침표로 끝낼 것."},{"title":"💡 소제목2","body":"5~6문장. 실전 커리어 적용법을 구체적으로. 반드시 카드 이름을 명시하며 그 카드가 나왔을 때 어떻게 활용하는지 설명할 것. 절대 이 카드 라고만 쓰지 말 것. 반드시 존댓말로, 마침표로 끝낼 것."},{"title":"💬 소제목3","body":"5~6문장. 실제 커리어 예시와 함께 설명. 반드시 카드 이름을 명시하며 그 카드가 어떤 의미인지 설명하고 나서 예시를 들 것. 절대 이 카드 라고만 쓰지 말 것. 반드시 존댓말로, 마침표로 끝낼 것."}],"extraCards":[${section.extraCards.map(c=>`{"file":"${c.file}","name":"${c.name}","interp":"커리어 리딩에서 한 줄 의미."}`).join(",")}],"badExamples":["예시1.","예시2.","예시3.","예시4."],"goodExamples":["예시1.","예시2.","예시3.","예시4."],"tableHeaders":["상황","정방향","역방향"],"tableRows":[{"col1":"상황1","col2":"정방향1","col3":"역방향1"},{"col1":"상황2","col2":"정방향2","col3":"역방향2"},{"col1":"상황3","col2":"정방향3","col3":"역방향3"},{"col1":"상황4","col2":"정방향4","col3":"역방향4"},{"col1":"상황5","col2":"정방향5","col3":"역방향5"}],"quote":"한 문장.","tip":"한 문장.","cases":[{"question":"${section.title}와 관련된 실제 커리어 상담에서 자주 나오는 질문 1. 반드시 실제 직업 고민 상황으로 카드 해석이 포함된 질문. 다른 케이스와 완전히 다른 상황으로.","answer":"존댓말로 3~4문장. 구체적인 카드 해석 포함. 반드시 마침표로 끝낼 것"},{"question":"${section.title}와 관련된 실제 커리어 상담에서 자주 나오는 질문 2. 앞의 질문과 완전히 다른 커리어 상황으로.","answer":"존댓말로 3~4문장. 구체적인 카드 해석 포함. 반드시 마침표로 끝낼 것"},{"question":"${section.title}와 관련된 실제 커리어 상담에서 자주 나오는 질문 3. 앞의 두 질문과 완전히 다른 커리어 상황으로.","answer":"존댓말로 3~4문장. 구체적인 카드 해석 포함. 반드시 마침표로 끝낼 것"}],"quiz":{"question":"🎯 퀴즈: 카드 3장 커리어 배열 제시.","hint":"힌트 한 줄.","answer":"2문장. 카드를 하나씩 나열하지 말고 전체 배열을 하나의 흐름으로 통합 해석. 모든 카드와 결론까지 반드시 포함. 내용이 풍부하고 빠지는 카드 없이. 반드시 존댓말로, 마침표로 끝낼 것."},"summary":["요약1.","요약2.","요약3.","요약4."]}`;
}
